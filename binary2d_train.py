...
import pdb
from utils import load_state_ignore_false_layers
...
class Train:
    ...
    def train(self):
        ...
        ## setup network
        netG = UNet_binary2d(nch_in, nch_out, nch_ker, norm)
        ...
        for m in netG.modules():
            if isinstance(m, BinaryConv2d):
                nn.init.constant_(m.weight, 0.51)

        netG.to(device)

        paramsG = netG.parameters()
        optimG = torch.optim.Adam(paramsG, lr=lr_G, betas=(beta1, 0.999))
        # optimG = torch.optim.SGD(paramsG, lr=lr_G)
        ...
        # load pretrain
        pretrained_model_path = '/home/shenxuan/project/UNet/pytorch_k_unet_master/resolution_384_same_mtk_input3channel/checkpoint/unet/em/model_epoch0500.pth'
        pretrained_model = torch.load(pretrained_model_path)['netG']
        netG = load_state_ignore_false_layers(netG, pretrained_model)
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for i, data in enumerate(loader_train, 1):
                ...
                # add binary conv2d
                loss_binary = append_binaryconv2d_loss(netG, percent=self.percent, alpha=self.alpha)
                loss_G_cls = loss_G_cls + loss_binary

                loss_G_cls.backward()

                # for name, weight in netG.named_parameters():
                #     if 'weight' in name and 'scale' in name:
                #         print(name)
                #         print(weight)
                #         # print(weight.grad)
                #         break
                ...
                print('TRAIN: EPOCH %d: BATCH %04d/%04d: CLS: %.4f:  BINARY_LOSS: %.4f'
                      % (epoch, i, num_batch_train, np.mean(loss_G_cls_train), loss_binary))
