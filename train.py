...
##
class Train:
    def __init__(self, args, args_ai=None):
        self.args = args
        ...
        self.dir_data = args.train_data_path
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader_val, 1):
                ...
                if should(num_freq_disp):
                    ...
                    plt.imsave(os.path.join(dir_result_val, 'images', fileset['output']), output[0].squeeze(),
                               cmap=cmap)

            writer_val.add_scalar('loss_G_cls', np.mean(loss_G_cls_val), epoch)

            return np.mean(loss_G_cls_val)
    ...
    def train(self):
        ...
        CL.init(args=self.args_ai, model=netG, optimizer=optimG, data_loader=loader_train)
        # CPL.init(args, model, optimizer)
        print_sparsity(netG, show_sparse_only=True)
        # Cocopie en
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for i, data in enumerate(loader_train, 1):
                ...
                loss_G_cls = CL.update_loss(loss_G_cls)
