...
class CNR2d(nn.Module):
    def __init__(self, nch_in, nch_out, kernel_size=4, stride=1, padding=1, norm='bnorm', relu=0.0, drop=[], bias=[]):
        ...
        # add binary conv2d
        self.cbr_scale = BinaryConv2d(nch_out, nch_out, kernel_size=1, stride=1, padding=0, groups=nch_out, bias=False)
...
class UnPooling2d(nn.Module):
    def __init__(self, nch=[], pool=2, type='nearest'):
        ...
        if type == 'nearest':
            ...
            self.unpooling = nn.ConvTranspose2d(nch*2, nch, kernel_size=pool, stride=pool)

            # add binary conv2d
            self.unpooling_scale = BinaryConv2d(nch, nch, kernel_size=1, stride=1, padding=0, groups=nch, bias=False)

    def forward(self, x):
        x = self.unpooling(x)
        x = self.unpooling_scale(x)
        return x
