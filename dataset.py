...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # shape_l = label.shape
        # shape_i = input.shape
        # label = label.reshape([shape_l[1], shape_l[2], shape_l[0]])
        # input = input.reshape([shape_i[1], shape_i[2], shape_i[0]])
        ...
        # sx
        # label = np.array(cv2.imread(os.path.join(self.data_dir, self.lst_label[index])))
        # input = np.array(cv2.imread(os.path.join(self.data_dir, self.lst_input[index])))
        #
        # label = label / 255.0
        # input = input / 255.0
