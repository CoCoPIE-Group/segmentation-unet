...
class UNet_binary2d(nn.Module):
    def __init__(self, nch_in, nch_out, nch_ker=64, norm='bnorm'):
        super(UNet_binary2d, self).__init__()
...
unet_list = [63700992.0, 339738624.0, 169869312.0, 339738624.0, 169869312.0, 339738624.0, 169869312.0, 339738624.0, 169869312.0, 339738624.0, 301989888.0, 679477248.0, 339738624.0, 301989888.0, 679477248.0, 339738624.0, 301989888.0, 679477248.0, 339738624.0, 301989888.0, 679477248.0, 339738624.0]

unet_mac_list = [1327104.0, 1327104.0, 331776.0, 331776.0, 82944.0, 82944.0, 20736.0, 20736.0, 5184.0, 5184.0, 9216.0, 20736.0, 20736.0, 36864.0, 82944.0, 82944.0, 147456.0, 331776.0, 331776.0, 589824.0, 1327104.0, 1327104.0]

# channel_list = []
# for i in range(len(unet_list)):
#     t = unet_list[i] / unet_mac_list[i]
#     channel_list.append(t*1e9)
#
# unet_list = channel_list
# unet_mac_list = [1e9]*len(unet_mac_list)

def append_binaryconv2d_loss(model, percent=0.66, alpha=5):
    alpha_adjust = alpha
    ...
    target_macs = torch.tensor(sum(unet_list) / 1e9).cuda()
    in_channel = torch.cat((torch.tensor([3]).cuda(), Branches[:-1]), dim=0)

    # unet special
    in_channel[11] = in_channel[11] + in_channel[8]
    in_channel[14] = in_channel[14] + in_channel[6]
    in_channel[17] = in_channel[17] + in_channel[4]
    in_channel[20] = in_channel[20] + in_channel[2]
    ...
    # print(in_channel)
    current_macs = torch.sum(torch.tensor(in_channel) * torch.tensor(unet_mac_list).cuda() * Branches) / 1e9
